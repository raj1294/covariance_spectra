...
inst = "fpma"
nuenergies = [1.65,3.0,3.5,4.0,5.0,6.0,7.0,8.0,10.0,12.0,\
              20.0,30.0,40.0,50.0,75.0]
nuenergies = np.array(nuenergies)
nuchans = (nuenergies - 1.6)/0.04
obsidnu = "30002037004"
nuenergies_begin = 1.65
nuenergies_end = 79.0

nuemin = nuenergies[0:-1]
nuemax = nuenergies[1:]
...
for nufile in sorted(glob.glob("nu*" + str(obsidnu) + "*" +\
                               str(inst) + "*cl.evt")):
        
    nuobsid = nufile.split("nu")[1].split(inst)[0][0:-1]

    for k in range(len(nuchans)-1):
        
        srcregfile = "src_" + obsidnu + "_" + str(inst) + ".reg"
        bkgregfile = "bkg_" + obsidnu + "_" + str(inst) + ".reg"

        nubegin1 = nuenergies_begin
        nuend1 = nuenergies[k]
        nubegin2 = nuenergies[k+1]
        nuend2 = nuenergies_end
        chanbegin1 = int((nubegin1 - 1.6)/0.04)
        chanend1 = int((nuend1 - 1.6)/0.04)
        chanbegin2 = int((nubegin2 - 1.6)/0.04)
        chanend2 = int((nuend2 - 1.6)/0.04)
        
        #Comparison-band light-curve
        comm0a = "read events " + nufile + " . "
        comm0b = "yes"
        comm0c = "filter pha_cutoff " + str(int(nuchans[k])) +\
                " " + str(int(nuchans[k+1]))      
        comm0d = "filter region " + srcregfile
        comm0e = "extract events"
        comm0f = "extract curve binsize=100"
        comm0g = "save curve nu" + str(nuobsid) + "_" + str(inst) + "_en" +\
                str(k+1) + "_comp.lc"
        comm0h = "clear all"
        comm0i = "yes"

        stringnulc.append(comm0a)
        stringnulc.append(comm0b)
        stringnulc.append(comm0c)
        stringnulc.append(comm0d)
        stringnulc.append(comm0e)
        stringnulc.append(comm0f)
        stringnulc.append(comm0g)
        stringnulc.append(comm0h)
        stringnulc.append(comm0i)

        evfnamep1 = "nu" + str(nuobsid) + "_" + str(inst) + "_en" +\
                    str(k+1) + "_p1.evt"
        evfnamep2 = "nu" + str(nuobsid) + "_" + str(inst) + "_en" +\
                    str(k+1) + "_p2.evt"
        lcfname = "nu" + str(nuobsid) + "_" + str(inst) + "_en" +\
                  str(k+1) + "_ref.lc"
        
        #Reference-band light-curve (part 1)
        comm1a = "read events " + nufile + " . "
        comm2a = "yes"
        comm3a = "filter pha_cutoff " + str(chanbegin1) +\
                 " " + str(chanend1)
        comm4a = "filter region " + srcregfile
        comm5a = "extract events"
        comm6a = "save events " + evfnamep1
        comm7a = "yes"
        comm8a = "clear all"
        comm9a = "yes"

        #Reference-band light-curve (part 2)
        comm1b = "read events " + nufile + " . "
        comm2b = "yes"
        comm3b = "filter pha_cutoff " + str(chanbegin2) +\
                 " " + str(chanend2)
        comm4b = "filter region " + srcregfile
        comm5b = "extract events"
        comm6b = "save events " + evfnamep2
        comm7b = "yes"
        comm8b = "clear all"
        comm9b = "yes"
        
        comm10 = "read events " + evfnamep1 + " . "
        comm11 = "read events " + evfnamep2
        comm12 = "extract curve binsize=100"
        comm13 = "save curve " + lcfname
        comm14a = "clear all"
        comm14b = "yes"

        stringnulc.append(comm1a)
        stringnulc.append(comm2a)
        stringnulc.append(comm3a)
        stringnulc.append(comm4a)
        stringnulc.append(comm5a)
        stringnulc.append(comm6a)
        stringnulc.append(comm7a)
        stringnulc.append(comm8a)
        stringnulc.append(comm9a)
        stringnulc.append(comm1b)
        stringnulc.append(comm2b)
        stringnulc.append(comm3b)
        stringnulc.append(comm4b)
        stringnulc.append(comm5b)
        stringnulc.append(comm6b)
        stringnulc.append(comm7b)
        stringnulc.append(comm8b)
        stringnulc.append(comm9b)
        stringnulc.append(comm10)
        stringnulc.append(comm11)
        stringnulc.append(comm12)
        stringnulc.append(comm13)
        stringnulc.append(comm14a)
        stringnulc.append(comm14b)
        stringnulc.append("")

# np.savetxt("nuextract.sh",stringnulc,fmt='%s',delimiter='  ')

############### Generate covariance spectrum ###################

keynu = "nu*fpma*comp*.lc"

covnu,dcovnu = [[],[]]
for nucomplcfile in sorted(glob.glob(keynu)):
                    
    # Comparison band light-curves
    hdulist = fits.open(nucomplcfile)    
    ...
    filterobs = 'NONE'
    detnam = hdr['DETNAM']
    tstartcomp = hdulist[2].data['START']
    tstopcomp = hdulist[2].data['STOP']
    
    timecomp = hdulist[1].data['TIME']
    ratecomp = hdulist[1].data['RATE']
    errorcomp = hdulist[1].data['ERROR']
    timeref = timecomp
    
    # Reference band light-curve
    nureflcfile = nucomplcfile.split("comp")[0] + "ref.lc"
    hduref = fits.open(nureflcfile)   
    timeref = hduref[1].data['TIME']
    rateref = hduref[1].data['RATE']
    errorref = hduref[1].data['ERROR']
    dtref = timeref[1]-timeref[0]
    tstartref = hduref[2].data['START']
    tstopref = hduref[2].data['STOP']
    ...
    bgrateref = np.zeros(len(rateref))
    errbgrateref = np.zeros(len(rateref))
    bgratecomp = np.zeros(len(ratecomp))
    errbgratecomp = np.zeros(len(ratecomp))
    ...
    ratecomp = ratecomp[isnanarr==False]
    errorcomp = errorcomp[isnanarr==False]
    bgrateref = bgrateref[isnanarr==False]
    bgratecomp = bgratecomp[isnanarr==False]
    
    timeref += tstartcomp[0]
    arraysW = np.transpose(np.column_stack((timeref,rateref)))
    windowref = rect_window(arraysW,tstartref,tstopref)
                
    # plt.errorbar(timeref/ks,rateref,yerr=errorref,fmt='k-')
    # plt.errorbar(timeref/ks,ratecomp,yerr=errorcomp,fmt='b-')
    # # plt.plot(timeref/ks,windowref,'r-')
    # plt.tick_params(axis='both', which='major', labelsize=14)
    # plt.legend(loc="best")
    # plt.title("NuSTAR lightcurve")
    # plt.ylabel("Count rate [s$^{-1}$]",fontsize=14)
    # plt.xlabel("Time [ks]",fontsize=14)
    # plt.show()
    
    ratecompbkg = np.zeros(len(ratecomp))
    errorcompbkg = np.zeros(len(ratecomp))
    ...
    covariance_spectrum(ratecomp,errorcomp,\
                        ...
                        statpow,fmi,fmx,windowref)
    covnu.append(intcov)
    dcovnu.append(intcoverr)
    
covnu = np.array(covnu)
dcovnu = np.array(dcovnu)
...
rmffile = "nu" + obsidnu + "_" + inst + "_sr.rmf"
ancrfile = "nu" + obsidnu + "_" + inst + "_sr.arf"
...
for j3 in range(len(nuemin)):
    for k3 in range(len(channel)):
            
        if(0.5*(EMIN[k3]+EMAX[k3])>=nuemax[j3]):
            fluxes[channel[k3]] = covnu[j3]
            dfluxes[channel[k3]] = dcovnu[j3]
            break

infile = "nucovflux_" + obsidnu + "_" + inst + ".dat"
Q = np.column_stack((chans,fluxes,dfluxes))
np.savetxt(infile,Q,fmt='%i %s %s',delimiter='   ')

#Unfold spectrum using instrumental response
outfile = "nucovspec_" + obsidnu + "_" + inst + ".pha"
specfile = "nu" + obsidnu + "_" + inst + "_sr.pha"
...
groupfile = "nucovspec_" + str(obsidnu) + "_grouped_" + inst + ".pha" 
